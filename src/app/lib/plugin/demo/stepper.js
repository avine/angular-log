class Stepper {
    constructor($root) {
        this.$root = $root;
        this.steps = []; // Register code step by step
        this.initDom();
        this.initEvent();
    ...
    initDom() {
        ...
        this.$logs = $wrapper.firstElementChild;
    ...
    cleanLog() {
        [].forEach.call(this.$logs.querySelectorAll('.helper-log'), $log => this.$logs.removeChild($log));
    }
    // Register a new step
    ...
    stepsHandler() {
        ...
        if (stepItem) {
            this.log(`<span class="helper-highlight">&#9679; ${stepItem.title}</span>`, '', stepItem.clean);
            stepItem.callback();
        }
    }
}
